#include "GameState.h"
#include <iostream>
GameState::GameState() : _rows(6), _cols(6)
{
    std::vector<Block> bg_layer(_rows, Block::Background);
    std::vector<Block> gr_layer(_rows, Block::Ground);
    std::vector<Block> layer(1, Block::Ground);
    for (int i = 0; i < _rows - 2;i++) {
        layer.push_back(Block::Background);
    }
    layer.push_back(Block::Ground);
...
void GameState::updatePos(std::string PlayerName,size_t sizeOfBlock,size_t x, size_t y) {
    auto& player = _players.find(PlayerName)->second;
    size_t X = x / sizeOfBlock;
    size_t Y = y / sizeOfBlock;

    if (_map[X][Y] == GameState::Block::Ground) {
        return;
    }
    else {
