...
unsigned long const kPeriod = 500; //ms
...
int main()
{
	...
	while (1)
	{
		...
		/*for (auto& map_pair : game.getListOfPlayers()) {
			if (map_pair.second.getStatus() == PlayerStatus::Active) {
				std::cout << '\n';
				std::cout << map_pair.second.getName() << " : ";
				std::cout << "x : " << size_t(map_pair.second.getPos().first) << " y = " << size_t(map_pair.second.getPos().second) << '\n';
			}
		}*/
		game.sendAll();
	}
